...
public class DoubleArrayList 
{
    ...
    public void addAll( DoubleArrayList addend )
    {
        ...
        for (int i = 0; i < addend.size(); i++) {
            elements[count2] = addend.get(count1);
            count1++;
            count2++;
            currentSize++;
        }
    ...
    public void addAll( int pos, DoubleArrayList addpos )
    {
        ...
        double[] temp = new double[size() - pos];
        int tempCount = 0;
        for (int i = pos; i < size(); i++ ) {
            temp[tempCount] = elements[i];
            tempCount++;
        }
        ...
        for (int i = 0; i < addpos.size(); i++) {
            elements[count2] = addpos.get(count1);
            currentSize++;
            count1++;
            count2++;
        }

        count1 = pos + addpos.size();
        count2 = 0;
        for (int i = 0; i < temp.length; i++) {
            elements[count1] = temp[count2];
            count1++;
            count2++;
        } 
